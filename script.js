...
class TruthTableGenerator {
    ...
    generateSubexpressions(expression, variables) {
        ...
        // Función para extraer subexpresiones de manera sistemática
        const extractSubexpressions = (expr) => {
            if (processed.has(expr)) return;
            processed.add(expr);
            
            // 1. Extraer negaciones simples (¬p, ¬q, etc.)
            for (let i = 0; i < expr.length; i++) {
                if (expr[i] === '¬' && i + 1 < expr.length && variables.includes(expr[i + 1])) {
                    const subexpr = '¬' + expr[i + 1];
                    if (!processed.has(subexpr)) {
            ...
            // 2. Extraer operaciones binarias simples (p∨q, p∧q, etc.)
            const operators = ['∧', '∨', '⊕', '→', '↔'];
            for (const op of operators) {
                for (let i = 0; i < expr.length; i++) {
                    if (expr[i] === op) {
                        // Buscar operando izquierdo (variable simple)
                        if (i > 0 && variables.includes(expr[i - 1])) {
                            // Buscar operando derecho (variable simple)
                            if (i + 1 < expr.length && variables.includes(expr[i + 1])) {
                                const subexpr = expr[i - 1] + op + expr[i + 1];
                                if (!processed.has(subexpr)) {
                                    subexpressions.push(subexpr);
                                    processed.add(subexpr);
                                }
            ...
            // 3. Extraer subexpresiones con paréntesis
            let depth = 0;
            let start = -1;
            ...
            for (let i = 0; i < expr.length; i++) {
                if (expr[i] === '(') {
                    ...
                    depth--;
                    if (depth === 0 && start !== -1) {
                        const subexpr = expr.substring(start + 1, i);
                        if (subexpr.length > 0 && !processed.has(subexpr)) {
                            subexpressions.push(subexpr);
                            processed.add(subexpr);
                            extractSubexpressions(subexpr);
                        }
                    }
                }
            }
            
            // 4. Extraer operaciones binarias complejas
            for (const op of operators) {
                let i = 0;
                while (i < expr.length) {
                    const opIndex = expr.indexOf(op, i);
                    if (opIndex === -1) break;
                    
                    // Verificar que no esté dentro de paréntesis
                    if (this.isInsideParentheses(expr, opIndex)) {
                        i = opIndex + 1;
                        continue;
                    }
                    
                    // Buscar operando izquierdo
                    const leftOperand = this.findLeftOperand(expr, opIndex, variables);
                    // Buscar operando derecho
                    const rightOperand = this.findRightOperand(expr, opIndex, variables);
                    
                    if (leftOperand && rightOperand && (leftOperand.length > 1 || rightOperand.length > 1)) {
                        const subexpr = leftOperand + op + rightOperand;
                        if (!processed.has(subexpr)) {
                            subexpressions.push(subexpr);
                            processed.add(subexpr);
                            extractSubexpressions(leftOperand);
                            extractSubexpressions(rightOperand);
                        }
                    }
        ...
        // Ordenar subexpresiones por orden de evaluación
        subexpressions.sort((a, b) => {
            // Primero por longitud
            if (a.length !== b.length) return a.length - b.length;
            // Luego por posición en la expresión original
            return expression.indexOf(a) - expression.indexOf(b);
        });
        
        // Agregar la expresión completa al final
